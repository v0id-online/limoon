...
function M.load(filename)
	...
	local MARK_BOOKMARK = limoon.bookmarks.MARK_BOOKMARK
...
-- `limoon.session.save_on_quit` is `false`.
...
function M.save(filename)
	...
	for _, buffer in ipairs(_BUFFERS) do
		...
		local bookmarks = {}
		local BOOKMARK_BIT = 1 << limoon.bookmarks.MARK_BOOKMARK - 1
