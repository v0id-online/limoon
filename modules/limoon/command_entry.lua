...
local env = setmetatable({}, {
	__index = function(_, k)
		...
		return buffer[k] or view[k] or ui[k] or _G[k] or limoon[k]
	end, --
...
-- In this environment, the contents of the `buffer`, `view`, `ui`, and `limoon` tables are
...
-- environment where the contents of the `buffer`, `view`, `ui`, and `limoon` tables are
...
local function complete_lua()
	...
	if not ok or symbol == 'buffer' or symbol == 'view' then
		...
		local global_envs = not ok and {buffer, view, ui, _G, limoon, sci} or {sci}
