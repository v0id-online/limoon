...
--- Copies text to the system clipboard using `limoon.clipboard.copy_command`.
-- @param text String text to copy.
local function copy(text)
	proc = os.spawn(limoon.clipboard.copy_command)
...
test('ui.get_clipboard_text should fall back on using its own internal clipboard', function()
	...
	local _<close> = test.mock(limoon.clipboard, 'copy_command', 'does-not-exist')
