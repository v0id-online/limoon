...
local function click(item) limoon.menu.menubar[item][2]() end
...
test('Edit > Complete Word should do so', function()
	...
	local _<close> = test.mock(limoon.editing, 'autocomplete', autocomplete)

	click('Edit/Complete Word')

	test.assert_equal(autocomplete.called, true)
	local autocompleter = autocomplete.args[1]
	test.assert(limoon.editing.autocompleters[autocompleter],
...
for name, auto_pair in pairs(enclosures) do
	test('Edit > Selection > Enclose in ' .. name .. ' should wrap selections', function()
		...
		local s = auto_pair
		local e = limoon.editing.auto_pairs[s]
...
test('Tools > Quick Open > Quickly Open Li Moon Home should do so', function()
	local quick_open = test.stub()
	local _<close> = test.mock(io, 'quick_open', quick_open)

	click('Tools/Quick Open/Quickly Open Li Moon Home')
...
test('limoon.menu.menubar should be mutable', function()
	local label = 'Extra Item'
	local item = {label, function() end}

	table.insert(limoon.menu.menubar, {title = 'Extra Menu', item})

	test.assert_equal(limoon.menu.menubar['Extra Menu'][label], item)
end)

test('limoon.menu.context_menu should be mutable', function()
	local label = 'Context Label'
	local item = {label, function() end}

	local _<close> = test.mock(limoon.menu, 'context_menu', {item})

	test.assert_equal(limoon.menu.context_menu[label], item)
end)

test('limoon.menu.tab_context_menu should be mutable', function()
	local label = 'Context Label'
	local item = {label, function() end}

	local _<close> = test.mock(limoon.menu, 'tab_context_menu', {item})

	test.assert_equal(limoon.menu.tab_context_menu[label], item)
end)
...
test('limoon.menu.menubar should be hideable', function()
	limoon.menu.menubar = nil -- hide

	limoon.menu.menubar = limoon.menu.menubar -- show

	test.assert(limoon.menu.menubar['File'] ~= nil, 'should not have cleared menubar')
end)

test('limoon.menu should still act like a table', function()
	local key = 'key'
	local value = 'value'

	local _<close> = test.mock(limoon.menu, key, value)

	test.assert_equal(limoon.menu[key], value)
end)
