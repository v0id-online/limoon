...
-- @usage limoon.run.compile_commands.c = 'clang -o "%e" "%f"'
...
-- from the `limoon.run.compile_commands` table.
...
-- default, the working directory is the current file's parent directory and the environment
-- is Li Moon's environment.
-- @usage limoon.run.run_commands.lua = 'lua5.1 "%f"'
-- @table run_commands
...
-- from the `limoon.run.run_commands` table.
-- The shell command is determined from the file's filename, extension, or language, in that order.
...
-- is Li Moon's environment.
-- @usage limoon.run.build_commands['CMakeLists.txt'] = 'cmake --build build'
-- @usage limoon.run.build_commands['/path/to/project'] = 'make -C src'
...
-- in. By default, the working directory is the project's root directory and the environment
-- is Li Moon's environment.
-- @usage limoon.run.test_commands['/path/to/project'] = 'pytest'
M.test_commands = {}
...
-- in. By default, the working directory is the project's root directory and the environment
-- is Li Moon's environment.
-- @usage limoon.run.run_project_commands[_HOME] = function()
...
-- `limoon.run.build_commands` table.
...
-- from the `limoon.run.test_commands` table.
...
--	command. The default value comes from `limoon.run.run_project_commands` and *dir*.
-- @see events.RUN_OUTPUT
