...
test('history.back should jump back to the last edit position when far enough away', function()
	local contents = test.lines(1 + limoon.history.minimum_line_distance + 1, true)
...
test('history.back should jump back to the last main edit pos for multiple selections', function()
	local word = 'word'
	local lines = {word}
	for _ = 1, limoon.history.minimum_line_distance do lines[#lines + 1] = '' end
	lines[#lines + 1] = word
...
test('history.back should not consider edits within history.minimum_line_distance as distinct',
	function()
		local contents = test.lines(1 + limoon.history.minimum_line_distance)
		local _<close> = test.tmpfile(contents, true)
		limoon.history.clear() -- start with a clean history
		test.type(' ')
		buffer:goto_line(1 + limoon.history.minimum_line_distance)
		test.type(' ')
		local last_edit_pos = buffer.current_pos
		buffer:document_start()

		limoon.history.back() -- should go back to last edit pos
		limoon.history.back() -- should not go back to first edit pos

		test.assert_equal(buffer.current_pos, last_edit_pos)
	end)

test('history.back should not be affected by reload, undo, or redo', function()
	local contents = test.lines(1 + limoon.history.minimum_line_distance + 1)
	local _<close> = test.tmpfile(contents, true)
	local last_edit_pos = buffer.current_pos
	test.type(' ')
...
test('history.forward should not return to a position without edits', function()
	...
	limoon.history.forward() -- should not go to document end

	test.assert_equal(buffer.current_pos, last_edit_pos)
end)

test('history should be finite', function()
	local _<close> = test.mock(limoon.history, 'maximum_history_size', 0)
	buffer.new()
	local f<close> = test.tmpfile(true)

	limoon.history.back() -- no history to go back to
...
test('history should be over-writeable', function()
	...
	limoon.history.forward() -- should remain in first file
...
test('history should be per-view', function()
	...
	limoon.history.back() -- no history
...
test('history.back/forward should update soft records', function()
	...
	limoon.history.back()
	buffer:document_end()
	limoon.history.forward() -- should update soft record in f1
	buffer:document_end()

	limoon.history.back() -- should update soft record in f2
...
test('typed buffer edits should always be soft records', function()
	...
	local chunk = test.lines(1 + limoon.history.minimum_line_distance + 1)
	ui.print_to(type)
	view:goto_buffer(-1)
