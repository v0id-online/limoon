...
-- @usage limoon.bookmarks.goto_mark(true) -- jump to the next bookmark
-- @usage limoon.bookmarks.goto_mark(false) -- jump to the previous bookmark
function M.goto_mark(next)
	if next ~= nil then
		...
		if line >= 1 then limoon.editing.goto_line(line) end
		return
	end
	...
	limoon.editing.goto_line(tonumber(utf8_list[i]:match('^[^:]+:(%d+):')))
end

local lines = {}
-- Save and restore bookmarks when replacing buffer text (e.g. buffer:reload(),
-- limoon.editing.filter_through()).
