...
test('macro.record should record keyboard macros', function()
	local _<close> = test.mock(keys, 'ctrl+,', limoon.macros.record)
	local _<close> = test.mock(keys, 'ctrl+.', limoon.macros.play)
	...
	test.type('home')
	test.type('down')
	limoon.macros.record() -- stop record (cannot use test.type() here)
...
test('macros.play should load and play the macro from a given filename', function()
	...
	record(function() test.type(' ') end)
	limoon.macros.save(f.filename)

	limoon.macros.play(f.filename)
