...
test('run.* should not prompt for a command to run if run.run_without_prompt is enabled', function()
	local _<close> = test.mock(limoon.run, 'run_without_prompt', true)
	local prompt = test.stub()
	local _<close> = test.mock(ui.command_entry, 'run', prompt)
	local f<close> = test.tmpfile()

	limoon.run.compile(f.filename)
...
test('run.compile/run should allow commands based on filename', function()
	...
	limoon.run.compile_commands[f.filename] = command
...
test('run.compile/run should allow commands based on file extension', function()
	...
	local command = 'command'
	local _<close> = test.mock(limoon.run.compile_commands, 'txt', command)
...
test('run.compile/run should allow commands based on lexer', function()
	...
	local _<close> = test.tmpfile(true)
	local command = 'command'
	local _<close> = test.mock(limoon.run.compile_commands, 'text', command)
...
test('run.* should mark recognized errors', function()
	...
	local line = buffer:marker_next(1, 1 << limoon.run.MARK_ERROR - 1)
...
test('run.* should silently run a command if run.run_in_background is enabled', function()
	local _<close> = test.mock(limoon.run, 'run_without_prompt', true)
	local _<close> = test.mock(limoon.run, 'run_in_background', true)
	local _<close> = test.mock(os, 'spawn', mock_spawn)
	local f<close> = test.tmpfile(true)
	limoon.run.compile_commands[f.filename] = 'echo compile'
...
local output_events = {
	[limoon.run.compile] = events.COMPILE_OUTPUT, --
	[limoon.run.run] = events.RUN_OUTPUT, --
	[limoon.run.build] = events.BUILD_OUTPUT, --
	[limoon.run.test] = events.TEST_OUTPUT, --
	[limoon.run.run_project] = events.RUN_OUTPUT
}

--- Invokes run function *f* for filename or directory *path*, optionally with command *command*,
-- and returns its string output.
-- @param f Run function to use (e.g. `limoon.run.run`).
...
test('run.run should run a run command for the current file', function()
	...
	local stdout = 'run'
	local command = 'echo ' .. stdout
	limoon.run.run_commands[f.filename] = command

	local output = mock_run(limoon.run.run)
...
test('run.* should allow functions to return commands and working dirs', function()
	...
	limoon.run.run_commands[f.filename] = command_in_dir

	local output = mock_run(limoon.run.run, f.filename)
...
test('run.* should allow functions to return commands, working dirs, and environments', function()
	...
	pcall(limoon.run.run, f.filename) -- assert(os.spawn(...)) will fail, which is fine
...
test('run.compile/run should allow macros in commands', function()
	local f<close> = test.tmpfile('.txt')
	limoon.run.run_commands[f.filename] = 'echo %p\t%d\t%f\t%e'
...
test('run.* should auto-update command tables per filename/directory', function()
	local f<close> = test.tmpfile()
	limoon.run.run_commands[f.filename] = 'echo 1'
	local command = 'echo 2'

	mock_run(limoon.run.run, f.filename, command)

	test.assert_equal(limoon.run.run_commands[f.filename], command)
end)
...
test('run.build should run a build command for the current project', function()
	...
	local stdout = 'build'
	local command = 'echo ' .. stdout
	limoon.run.build_commands[dir.dirname] = command

	local output = mock_run(limoon.run.build)
...
test('run.build should allow commands based on top-level files', function()
	...
	local _<close> = test.mock(limoon.run.build_commands, file, command)

	local output = mock_run(limoon.run.build, dir.dirname)
...
test('run.test should run a test command for the current project', function()
	...
	local stdout = 'test'
	local command = 'echo ' .. stdout
	limoon.run.test_commands[dir.dirname] = command

	local output = mock_run(limoon.run.test)
...
test('run.test should run a project command for the current project', function()
	...
	local command = 'echo ' .. stdout
	limoon.run.run_project_commands[dir.dirname] = command

	local output = mock_run(limoon.run.run_project)
...
test('run.* should send the output buffer line as stdin on Enter', function()
	...
	local limoon_exe = lfs.abspath(arg[0])
	limoon.run.run_commands[f.filename] = limoon_exe .. ' -L "%f"'
	limoon.run.run(f.filename)
...
test('run.goto_error(true) should go to the next error/warning found', function()
	...
	limoon.run.run_commands[f.filename] = command .. ' "%f"'
	limoon.run.run()

	limoon.run.goto_error(true)
