...
-- used, if available.
-- @usage limoon.editing.comment_string.c = '/*|*/' -- instead of the default '//'
M.comment_string = {}
...
-- @usage limoon.editing.auto_pairs['*'] = '*'
-- @usage limoon.editing.auto_pairs = nil -- disable completely
M.auto_pairs = {}
for k, v in string.gmatch([[()[]{}''""``]], '(.)(.)') do M.auto_pairs[k] = v end

--- Type over an auto-paired complement character from `limoon.editing.auto_pairs`.
...
-- `limoon.editing.auto_pairs` into account.
...
--- Comments or uncomments source lines based on `limoon.editing.comment_string`.
...
--	`limoon.editing.auto_pairs` and inferred from the current position or selection.
...
-- @usage limoon.editing.filter_through('sort | uniq') -- sort lines and remove duplicates
...
-- @param name The name of an autocompleter function in the `limoon.editing.autocompleters`
...
-- `limoon.editing.autocomplete_all_words` is `true`.
...
-- @function _G.limoon.editing.autocompleters.word
...
events.connect(events.KEYPRESS, function(key)
	...
	if limoon.snippets.active and not M.auto_pairs[key] then return end -- likely placeholder
