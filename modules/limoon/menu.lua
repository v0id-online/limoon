...
local function change_case(upper)
	...
	if select then limoon.editing.select_word() end
...
-- @usage table.insert(limoon.menu.menubar['Tools'], {...}) -- Append to the Tools menu
-- @usage limoon.menu.menubar['File/New'] --> table for "File > New"
-- @usage limoon.menu.menubar['File/New'][2] = function() ... end -- change "File > New" command
-- @table menubar
...
local default_menubar = {
	{
		...
		}, {_L['Complete Word'], function() limoon.editing.autocomplete('word') end},
		{_L['Toggle Block Comment'], limoon.editing.toggle_comment},
		{_L['Join Lines'], limoon.editing.join_lines}, {
			_L['Filter Through'], function()
				ui.command_entry.run(_L['Shell command:'], limoon.editing.filter_through, 'bash')
			end
		}, {
			title = _L['Select'],
			{_L['Select between Matching Delimiters'], limoon.editing.select_enclosed},
			{_L['Select Word'], limoon.editing.select_word},
			{_L['Deselect Word'], function() buffer:drop_selection_n(buffer.selections) end},
			{_L['Select Line'], limoon.editing.select_line},
			{_L['Select Paragraph'], limoon.editing.select_paragraph}
		}, {
			title = _L['Selection'], --
			...
			}, {_L['Enclose as Single XML Tag'], function() limoon.editing.enclose('<', ' />') end},
			{_L['Enclose in Single Quotes'], function() limoon.editing.enclose("'", "'") end},
			{_L['Enclose in Double Quotes'], function() limoon.editing.enclose('"', '"') end},
			{_L['Enclose in Parentheses'], function() limoon.editing.enclose('(', ')') end},
			{_L['Enclose in Brackets'], function() limoon.editing.enclose('[', ']') end},
			{_L['Enclose in Braces'], function() limoon.editing.enclose('{', '}') end}, --
			SEPARATOR, --
			{_L['Move Selected Lines Up'], buffer.move_selected_lines_up},
			{_L['Move Selected Lines Down'], buffer.move_selected_lines_down}
		}, {
			title = _L['History'], --
			{_L['Navigate Backward'], limoon.history.back},
			{_L['Navigate Forward'], limoon.history.forward},
			{_L['Record Location'], limoon.history.record}, --
			SEPARATOR, --
			{_L['Clear History'], limoon.history.clear}
		}, SEPARATOR, --
		...
		SEPARATOR, --
		{_L['Go To Line...'], limoon.editing.goto_line}
	}, {
		title = _L['Tools'], --
		{_L['Command Entry'], ui.command_entry.run}, --
		{_L['Select Command'], M.select_command}, --
		SEPARATOR, --
		{_L['Run'], limoon.run.run}, --
		{_L['Compile'], limoon.run.compile}, --
		{_L['Build'], limoon.run.build}, --
		{_L['Run tests'], limoon.run.test}, --
		{_L['Run project'], limoon.run.run_project}, --
		{_L['Stop'], limoon.run.stop},
		{_L['Next Error'], function() limoon.run.goto_error(true) end},
		{_L['Previous Error'], function() limoon.run.goto_error(false) end}, --
		SEPARATOR, --
		{
			title = _L['Bookmarks'], --
			{_L['Toggle Bookmark'], limoon.bookmarks.toggle},
			{_L['Clear Bookmarks'], limoon.bookmarks.clear},
			{_L['Next Bookmark'], function() limoon.bookmarks.goto_mark(true) end},
			{_L['Previous Bookmark'], function() limoon.bookmarks.goto_mark(false) end},
			{_L['Go To Bookmark...'], limoon.bookmarks.goto_mark}
		}, {
			title = _L['Macros'], --
			{_L['Start/Stop Recording'], limoon.macros.record}, --
			{_L['Play'], limoon.macros.play}, --
			SEPARATOR, --
			{_L['Save...'], limoon.macros.save}, --
			{_L['Load...'], limoon.macros.load}
		}, {
			title = _L['Quick Open'],
			{_L['Quickly Open User Home'], function() io.quick_open(_USERHOME) end},
			{_L['Quickly Open Li Moon Home'], function() io.quick_open(_HOME) end}, {
			...
			title = _L['Snippets'], --
			{_L['Insert Snippet...'], limoon.snippets.select},
			{_L['Expand Snippet/Next Placeholder'], limoon.snippets.insert},
			{_L['Previous Snippet Placeholder'], limoon.snippets.previous},
			{_L['Cancel Snippet'], limoon.snippets.cancel}, --
			SEPARATOR, --
			{_L['Complete Trigger Word'], function() limoon.editing.autocomplete('snippet') end}
		}, SEPARATOR, --
		...
		SEPARATOR, --
		{
			...
			}, {_L['Convert Indentation'], limoon.editing.convert_indentation}
		}, {
			title = _L['EOL Mode'], --
		...
		SEPARATOR, {
			_L['About'],
			function() ui.dialogs.message{title = _RELEASE, text = _COPYRIGHT, icon = 'limoon'} end
		}
	}
}
if QT and OSX then
...
-- @usage table.insert(limoon.menu.context_menu, {'Label', function() ... end})
-- @table context_menu
...
--	menubar from view, but keeps it intact in order for `limoon.menu.select_command()`
