...
// Variables declared in limoon.h.
char *limoon_home;
...
bool emit(const char *name, ...) {
	...
	for (int type = va_arg(ap, int); type != -1; type = va_arg(ap, int), n++) switch (type) {
		...
		case LUA_TSTRING: lua_pushstring(lua, (const char*)va_arg(ap, char *)); break;
...
// Runs the given Lua file, which is relative to `limoon_home`, and returns `true` on success.
...
static bool init_lua(int argc, char **argv) {
	...
	// Check for invoking Li Moon as a Lua interpreter.
	...
	lua_pushstring(L, limoon_home), lua_setglobal(L, "_HOME");
...
#error platform not supported
#endif
#ifdef LIMOON_HOME
	snprintf(limoon_home, FILENAME_MAX + 1, "%s", LIMOON_HOME);
#endif
	if (getenv("LIMOON_HOME")) snprintf(limoon_home, FILENAME_MAX + 1, "%s", getenv("LIMOON_HOME"));

	setlocale(LC_COLLATE, "C"), setlocale(LC_NUMERIC, "C"); // for Lua
	bool ok = init_lua(argc, argv);
	if (!ok) return (close_limoon(), ok); // exit_status has been set
	...
	if (!ok) return (close_limoon(), exit_status = 1, ok);
