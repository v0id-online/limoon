...
// Interface between Li Moon and platforms.
#ifndef LIMOON_PLATFORM_H
#define LIMOON_PLATFORM_H
// Li Moon calls these functions to communicate with the platform.
//
// Platforms are expected to implement the `main()` entry point of the program, perform any
// platform-specific initialization required on startup, call `init_limoon()` to initialize
// Li Moon (which in turn creates the window, etc.), and then start the main event loop. Prior
// to exiting the main loop, platforms will typically want to call `close_limoon()` to clean
...
/** Asks the platform to create the Li Moon window.
 ...
 * window. Li Moon will call on the platform to fill in more details later, such as setting
 ...
 *
 * Note that Li Moon does not create one view per tab, so platforms are expected to keep the
...
/** Sets the title of the Li Moon window to the given text. */
void set_title(const char *title);
/** Returns whether or not the Li Moon window is maximized. */
bool is_maximized(void);
/** Sets the maximized state of the Li Moon window. */
void set_maximized(bool maximize);
/** Stores the current width and height of the Li Moon window into the given variables. */
void get_size(int *width, int *height);
/** Sets the width and height of the Li Moon window to the given values. */
...
/** Sets whether or not the Li Moon window should show tabs for its buffers. */
void show_tabs(bool show);
/** Asks the platform to add a tab to the end of its tab list.
 * The platform is not expected to attach anything to the tab, as Li Moon does not create
 * one view per tab.
 */
void add_tab(void);
 ...
 * As Li Moon does not have one view per tab, the platform is not expected to manage any
 ...
 *
 * Li Moon will store the returned menu and either pass it to `popup_menu()` or put it in a
 ...
 * The platform must return an allocated string -- Li Moon will call free() on it.
 */
 ...
 * When the process exits, it needs to notify Li Moon via `process_exited()`.
 ...
 *	process environment. If this index is not provided, the child should inherit Li Moon's
 *	environment.
 * @param monitor_stdout Whether or not the platform should notify Li Moon of any process
 *	stdout as it comes in via `process_output()`.
 * @param monitor_stderr Whether or not the platform should notify Li Moon of any process
 ...
 * Li Moon will only call this after the platform has indicated the process exited.
 */
...
/** Asks the platform to suspend execution of Li Moon, if possible. */
void suspend(void);
